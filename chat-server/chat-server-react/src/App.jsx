...
function App() {
  ...
  const getMessages = async () => {
    try {
      // Make GET request
      const getResponse = await axios.get("http://localhost:9090/messages");
  ...
  const handleClose = async (id) => {
    try {
      await fetch(`http://localhost:9090/messages/${id}`, {
        method: "DELETE",
      });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:9090/messages", {
        id: count,
        from: from,
        text: text,
        time: currentTime,
